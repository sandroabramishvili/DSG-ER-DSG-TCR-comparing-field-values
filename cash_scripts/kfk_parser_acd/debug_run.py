...
parse_from_file_flag = None
write_input_flag = 1
parse_from_kfk_flag = 1
enr_flag = 1
...
try:
    modules.make_enrichment.get_enrichment(max_vers,parse_from_kfk_flag,parse_from_file_flag,today)
except:
...
if clean_directory_flag:
  ...
  if os.path.exists('{}.tar.gz'.format(input_dir)):
    os.remove('{}.tar.gz'.format(input_dir))
      ...
      for pdx in parts:
        ...
        for idx, msg in enumerate(consumer):
          ...
          try:
              ...
              print('Dictionary for {} version does not exists. Using dictionary for {} version'.format(vers,max_vers))
              msg_dict = imp.import_module('modules.dict.dict_{}'.format(max_vers))
              vers = max_vers
          if write_input_flag:
              with open("{}/{}.dat".format(input_dir,topic), 'a') as f:
                f.write("{},{},{},{},{},{}\n".format(msg.timestamp,msg_dict.topicid[msg.topic],msg.partition,msg.offset,vers, msg.value.hex()))
